...
- **Q17** Publishing: `@kisaes/vibe-auth` to GitHub Packages requires the `kisaes` org/repo to exist; the sibling repos use `KisaesDevLab` (`ghcr.io/kisaesdevlab/*`). Which owner: `kisaes` (per plan) or `KisaesDevLab` (per every other repo)? Built with `kisaes`; CI reads `GITHUB_REPOSITORY` so it follows wherever the repo lives. The Appliance manifest/overlay reference `ghcr.io/kisaes/vibe-auth` — change with the answer.
- **Q18** §2.3 item 5 says the console refuses `oidc_only` unless "a test login succeeded in the current console session". The console cannot observe product sessions, so **built:** the console guard is "break-glass password stored"; the "successful test login in this session (60 min, same admin)" guard is enforced by the product's own Settings → Authentication page (package settings API). Acceptable split?
- **Q19** Sibling-repo changes are uncommitted on purpose: Phase 6 (Vibe-Appliance: schema, manifest, overlay, env template, `lib/identity.sh`, renderer, console Identity panel, bootstrap ordering, CLI) and Phase 8 step 1 (trial-balance-app). Shall I commit them on branches `vibe-auth-integration` in each repo, or open PRs from those branches?
- **Q20** One Appliance test (`tests/routing/subdomain-per-app.test.js:442`) fails on a clean checkout of HEAD on this Windows box too (`python3` via `execFileSync`) — pre-existing/environmental, not touched. Ignore?
