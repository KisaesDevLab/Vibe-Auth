...
### Break-glass, role sync and the CLI (1.0.6)
`vibe-auth breakglass ensure|rotate|status|verify [--json]` runs inside the product container (the Appliance `docker exec`s it). `ensure` on an existing active account changes nothing and returns no password; it repairs absence only. `status` reports `exists / active / admin / ready / problems[]` and folds in the product's own rules through `VibeAuthCliAdapter.breakglassCheck` (second factor enrolled, locked, forced password change). `verify` reads a password on **stdin** and reports `{checked, matches}` through the optional `UserAdapter.verifyLocalPassword`, which is how the Appliance notices a stored password that a database restore has separated from the account.

The engine identifies break-glass by username **or** by `breakglassEmail` (`VibeAuthOptions`, else `VIBE_BREAKGLASS_EMAIL`, else `<username>@vibe-auth.local`; never `@localhost`, which most login validators reject): `isBreakglassIdentifier`, `localLoginAllowed`, `afterLocalLogin`. `defaultRoleMapFor` matches case-insensitively and leaves a group unmapped rather than guessing; `engine.start()` warns when a product relies on it and groups are unmapped. Role sync refuses to demote the last active admin (`countOtherActiveAdmins`, optional) and honours `setRole` returning `false`; both are audited as `vibe.auth.role.changed` with `refused: true`. `login.success` is audited only after `session.create()` returns; a throwing SessionAdapter is a `login.failure` (`session_failed`) with the normal error page. Adapter types are framework-neutral (`SessionAdapter<Req, Res>`) and `express` is an optional peer. `await auth.ready()` waits for the first discovery attempt.
