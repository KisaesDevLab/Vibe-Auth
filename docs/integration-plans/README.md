...
| 10 | Recap | `vibe-recap.md` | Fastify + Postgres sessions | 2 h | implemented 2026-09-20 on `Vibe-Recap` main (`3b876b2`, `a7ba1c6`), by another session; already meets the hardening checklist in `break-glass-and-rollout-risks.md` (dotted break-glass address, absolute CLI path, protected account, SSO-only reset refused, last-admin guard, explicit role map). **Not released** (latest tag v0.3.0 predates it) and **exit gate not met** (no real-browser sign-in). Appliance manifest added on `Vibe-Appliance` `feat/identity-full-management` (`e861f80`) |
