...
| I7 | `UserAdapter.create` (JIT) writes an unusable random password hash, marks the email verified where the schema has such a column, and never sets `must_change_password`. `createLocalUser` uses the product's own hash algorithm and cost. | Phase 3 |
| I8 | Roles: map Vibe groups to the product's **role slugs only** (D22, D29 defaults); never touch per-user permission overrides (T&B, Investments, Calculators, MyBooks). `syncRoles: true`. | D22 |
...
**B. UserAdapter** (`src/lib/vibeAuthUsers.ts`): reference `trial-balance-app/server/src/lib/vibeAuthUsers.ts` (int ids, username-based) and `Vibe-Sentinel/apps/api/src/lib/vibeAuthUsers.ts` (uuid, email-based). Map the product row → `VibeUser`; implement all eight methods; `findByUsername` for email-only products maps `vibe-breakglass` → `vibe-breakglass@localhost`.
